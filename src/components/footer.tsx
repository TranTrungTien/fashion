import {
  FacebookOutlined,
  GoogleOutlined,
  TwitterOutlined,
  YoutubeOutlined,
} from "@ant-design/icons";
import React from "react";

const Footer = () => {
  return (
    <div className="footer">
      <div className="footer-static-top">
        <div className="container">
          <div className="footer-shipping pt-60 pb-55 pb-xs-25">
            <div className="row">
              <div className="col-lg-3 col-md-6 col-sm-6 pb-sm-55 pb-xs-55">
                <div className="li-shipping-inner-box">
                  <div className="shipping-icon">
                    <img
                      src="/images/shipping-icon/1.png"
                      alt="Shipping Icon"
                    />
                  </div>
                  <div className="shipping-text">
                    <h2>Giao hàng miễn phí</h2>
                    <p>
                      Và trả lại hàng miễn phí. Xem phần thanh toán để biết ngày
                      giao hàng.
                    </p>
                  </div>
                </div>
              </div>
              <div className="col-lg-3 col-md-6 col-sm-6 pb-sm-55 pb-xs-55">
                <div className="li-shipping-inner-box">
                  <div className="shipping-icon">
                    <img
                      src="/images/shipping-icon/2.png"
                      alt="Shipping Icon"
                    />
                  </div>
                  <div className="shipping-text">
                    <h2>Thanh toán an toàn</h2>
                    <p>
                      Thanh toán bằng phương thức thanh toán an toàn và phổ biến
                      nhất thế giới.
                    </p>
                  </div>
                </div>
              </div>
              <div className="col-lg-3 col-md-6 col-sm-6 pb-xs-30">
                <div className="li-shipping-inner-box">
                  <div className="shipping-icon">
                    <img
                      src="/images/shipping-icon/3.png"
                      alt="Shipping Icon"
                    />
                  </div>
                  <div className="shipping-text">
                    <h2>Tự Tin Mua Sắm</h2>
                    <p>
                      Bảo vệ người mua của chúng tôi bao gồm giao dịch mua của
                      bạn từ nhấp chuột đến giao hàng.
                    </p>
                  </div>
                </div>
              </div>
              <div className="col-lg-3 col-md-6 col-sm-6 pb-xs-30">
                <div className="li-shipping-inner-box">
                  <div className="shipping-icon">
                    <img
                      src="/images/shipping-icon/4.png"
                      alt="Shipping Icon"
                    />
                  </div>
                  <div className="shipping-text">
                    <h2>Trung tâm trợ giúp 24/7</h2>
                    <p>
                      Bạn có câu hỏi? Gọi cho Chuyên gia hoặc trò chuyện trực
                      tuyến.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="footer-static-middle">
        <div className="container">
          <div className="footer-logo-wrap pt-50 pb-35">
            <div className="row">
              <div className="col-lg-4 col-md-6">
                <div className="footer-logo">
                  <img src="/images/logo.jpg" className="h-[45px]" />
                  <p className="info"></p>
                </div>
                <ul className="des">
                  <li>
                    <span>Địa chỉ: </span>
                    321 VN
                  </li>
                  <li>
                    <span>Điện thoại: </span>
                    <a href="#">(+123) 123 321 345</a>
                  </li>
                  <li>
                    <span>Email: </span>
                    <a href="mailto://info@yourdomain.com">info@xxx.com</a>
                  </li>
                </ul>
              </div>
              <div className="col-lg-2 col-md-3 col-sm-6">
                <div className="footer-block">
                  <h3 className="footer-block-title">Sản phẩm</h3>
                  <ul>
                    <li>
                      <a href="#">Giá giảm</a>
                    </li>
                    <li>
                      <a href="#">Sản phẩm mới</a>
                    </li>
                    <li>
                      <a href="#">Bán hàng tốt nhất</a>
                    </li>
                    <li>
                      <a href="#">Liên hệ với chúng tôi</a>
                    </li>
                  </ul>
                </div>
              </div>
              <div className="col-lg-2 col-md-3 col-sm-6">
                <div className="footer-block">
                  <h3 className="footer-block-title">Công ty chúng tôi</h3>
                  <ul>
                    <li>
                      <a href="#">Giao hàng</a>
                    </li>
                    <li>
                      <a href="#">Thông báo pháp lý</a>
                    </li>
                    <li>
                      <a href="#">Giới thiệu</a>
                    </li>
                    <li>
                      <a href="#">Liên hệ với chúng tôi</a>
                    </li>
                  </ul>
                </div>
              </div>
              <div className="col-lg-4">
                <div className="footer-block">
                  <h3 className="footer-block-title">Hãy theo dõi chúng tôi</h3>
                  <ul className="social-link">
                    <li className=" text-xl ">
                      <a
                        className="text-xl"
                        href="https://twitter.com/"
                        data-toggle="tooltip"
                        target="_blank"
                        title="Twitter"
                      >
                        <TwitterOutlined />
                      </a>
                    </li>
                    <li className=" text-xl">
                      <a
                        className="text-xl"
                        href="https://www.plus.google.com/discover"
                        data-toggle="tooltip"
                        target="_blank"
                        title="Google +"
                      >
                        <GoogleOutlined />
                      </a>
                    </li>
                    <li className=" text-xl ">
                      <a
                        className="text-xl"
                        href="https://www.facebook.com/"
                        data-toggle="tooltip"
                        target="_blank"
                        title="Facebook"
                      >
                        <FacebookOutlined />
                      </a>
                    </li>
                    <li className=" text-xl ">
                      <a
                        className="text-xl"
                        href="https://www.youtube.com/"
                        data-toggle="tooltip"
                        target="_blank"
                        title="Youtube"
                      >
                        <YoutubeOutlined />
                      </a>
                    </li>
                    <li className=" text-xl">
                      <a
                        className="text-xl"
                        href="https://www.instagram.com/"
                        data-toggle="tooltip"
                        target="_blank"
                        title="Instagram"
                      >
                        <TwitterOutlined />
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="footer-newsletter">
                  <h4>Đăng ký nhận bản tin</h4>
                  <div id="mc_embed_signup_scroll">
                    <div
                      id="mc-form"
                      className="mc-form subscribe-form form-group"
                    >
                      <input
                        id="mc-email"
                        type="email"
                        autoComplete="off"
                        placeholder="Email"
                      />
                      <button className="btn" id="mc-submit">
                        Đăng ký
                      </button>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="footer-static-bottom pt-55 pb-55">
        <div className="container">
          <div className="row">
            <div className="col-lg-12">
              <div className="footer-links">
                <ul>
                  <li>
                    <a href="#">Mua sắm trực tuyến</a>
                  </li>
                  <li>
                    <a href="#">Khuyến mại</a>
                  </li>
                  <li>
                    <a href="#">Đơn hàng của tôi</a>
                  </li>
                  <li>
                    <a href="#">Trợ giúp</a>
                  </li>
                  <li>
                    <a href="#">Dịch vụ khách hàng</a>
                  </li>
                  <li>
                    <a href="#">Hỗ trợ</a>
                  </li>
                  <li>
                    <a href="#">Phổ biến nhất</a>
                  </li>
                  <li>
                    <a href="#">Hàng mới về</a>
                  </li>
                  <li>
                    <a href="#">Sản phẩm đặc biệt</a>
                  </li>
                  <li>
                    <a href="#">Nhà sản xuất</a>
                  </li>
                  <li>
                    <a href="#">Cửa hàng của chúng tôi</a>
                  </li>
                  <li>
                    <a href="#">Vận chuyển</a>
                  </li>
                  <li>
                    <a href="#">Thanh toán</a>
                  </li>
                  <li>
                    <a href="#">Bảo hành</a>
                  </li>
                  <li>
                    <a href="#">Hoàn tiền</a>
                  </li>
                  <li>
                    <a href="#">Thanh toán</a>
                  </li>
                  <li>
                    <a href="#">Giảm giá</a>
                  </li>
                  <li>
                    <a href="#">Hoàn tiền</a>
                  </li>
                  <li>
                    <a href="#">Chính sách vận chuyển</a>
                  </li>
                </ul>
              </div>
              <div className="copyright text-center">
                <a href="#">
                  <img src="images/payment/1.png" alt="" />
                </a>
              </div>
              <div className="copyright text-center pt-25">
                <span>copyright © 2022 all rights reserved</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Footer;
